import Navbar from './NavbarAuthorized';
...
export default function Dashboard(props) {
    ...
    let sortedFinData = finData;
    sortedFinData.sort((a,b) => {
        if (a.value > b.value) {
            return -1;
    ...
    const total = finData.reduce((sum, item) => sum+item.value, 0)
            ...
            <div className='xl:flex align-middle mx-7 xl:mx-10'>
                <div className='xl:basis-1/2 m-3'> {/* pie chart container*/} 
                    <ResponsiveContainer width='100%' style={{aspectRatio:'1/1'}}>
                        <PieChart>
                            <Pie dataKey="value" cx='50%' cy='50%' data={finData} label={ ({name}) => `${name}`} innerRadius={'45%'} outerRadius={'70%'}>
                ...
                <div className='xl:basis-1/2 m-4'>
                    ...
                    {sortedFinData.slice(0,5).map((item, idx) => {
